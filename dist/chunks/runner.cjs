...
const path__default = /*#__PURE__*/_interopDefaultLegacy(path$3);
...
const os__default$1 = /*#__PURE__*/_interopDefaultLegacy(os$2);
...
function cmdExists(cmd) {
  try {
    require$$0$1.execSync(
      os__default.platform() === "win32" ? `cmd /c "(help ${cmd} > nul || exit 0) && where ${cmd} > nul 2> nul"` : `command -v ${cmd}`
    );
    return true;
  } catch {
    return false;
...
const parseNi = (agent, args, ctx) => {
  if (args.length === 1 && args[0] === "-v") {
    console.log(`@joruno/ni v${version}`);
    process.exit(0);
  }
  if (agent === "bun")
...
const defaultRcPath = require$$0__default.join(home || "~/", ".nirc");
...
async function getConfig() {
  if (!config) {
    ...
    if (agent)
      ...
      config = Object.assign({}, defaultConfig, ini.parse(fs__default.readFileSync(rcPath, "utf-8")));
  }
  return config;
}
...
if (!processOk(process$1)) {
  ...
  var assert = require$$0__default$2;
...
const isSigterm = signal => signal === os__default$1.constants.signals.SIGTERM
...
const {constants: BufferConstants} = require$$0__default$4;
...
async function detect({ autoInstall, cwd }) {
  ...
  let packageJsonPath;
  if (lockPath)
    packageJsonPath = require$$0__default.resolve(lockPath, "../package.json");
  else
  ...
  if (!agent && lockPath)
    agent = LOCKS[require$$0__default.basename(lockPath)];
